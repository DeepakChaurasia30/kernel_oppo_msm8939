...
#ifdef CONFIG_MACH_OPPO
/*Xiongxing@Prd6.BaseDrv.Camera, 20170221, add for different led*/
static int request_led_gpio_state(int pin) {
	int state = -1;
	//int err = 0;
...
static ssize_t flash_led_age_read(struct file *filp, char __user *buff,
                        	size_t len, loff_t *data) {
    char value[2] = {0};
...
/*Xiongxing@Prd6.BaseDrv.Camera, 20170221, add for different led*/
	flash_age = FLASH_OLD;
	//get gpio50 state
	flash_age = request_led_gpio_state(952);
	if (flash_age >= 0) {

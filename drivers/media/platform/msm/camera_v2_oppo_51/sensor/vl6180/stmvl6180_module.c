...
static void stmvl6180_read_calibration_file(void)
{
	...
	f = filp_open("/persist/LaserFocusOffset.txt", O_RDONLY, 0);
	...
	is_sign=0;
	f = filp_open("/persist/LaserFocusCrossTalk.txt", O_RDONLY, 0);
	if (f!= NULL && !IS_ERR(f) && f->f_dentry!=NULL)
	{
		fs = get_fs();
		set_fs(get_ds());
...
//offset_calib is int8_t type, but the data is saved as unsigned int, so need to convert
	...
	f = filp_open("/persist/LaserFocusOffset.txt", O_WRONLY|O_CREAT, 0644);
	if (f!= NULL)
	{
		fs = get_fs();
		set_fs(get_ds());
		...
		pr_err("%s:%d open /persist/LaserFocusOffset.txt failed\n", __func__, __LINE__);
...
//cross talk is always positive
	...
	f = filp_open("/persist/LaserFocusCrossTalk.txt", O_WRONLY|O_CREAT, 0644);
	if (f!= NULL)
	{
		fs = get_fs();
		set_fs(get_ds());
		...
		pr_err("%s:%d open /persist/LaserFocusCrossTalk.txt failed\n", __func__, __LINE__);
...
static void stmvl6180_ps_notify_abort(void)
{
	...
	do_gettimeofday(&tv);
	vl6180_data->ps_data = vl6180_data->rangeData.range_mm;
	//notify that get device abort
	vl6180_data->rangeData.errorStatus = 0xdeaddead;
	input_report_abs(vl6180_data->input_dev_ps, ABS_DISTANCE, (int)(vl6180_data->ps_data+5)/10); //range in cm
	input_report_abs(vl6180_data->input_dev_ps, ABS_HAT0X,tv.tv_sec);
	input_report_abs(vl6180_data->input_dev_ps, ABS_HAT0Y,tv.tv_usec);
	input_report_abs(vl6180_data->input_dev_ps, ABS_HAT1X,vl6180_data->rangeData.range_mm);
...
static ssize_t vendor_enable_ps_sensor(int enable)
{
	struct stmvl6180_data *vl6180_data = vl6180_data_g;
	int rc = 0;
	CDBG("%s:enable %d\n",__func__, enable);
...
//likelong added
	...
	if(ret < 0){
		...
		if(vl6180_data->force_reset_cnt++ > 3){
			stmvl6180_ps_notify_abort();
			mutex_unlock(&vl6180_data->work_mutex); //#tt999
			return;
		}
		printk("%s:reset ret=%d RESULT_RANGE_STATUS = %d \n",__func__,ret, vl6180_data->ResultBuffer[0]);
		vendor_enable_ps_sensor(0);
		msleep(100);
		vendor_enable_ps_sensor(1);
		schedule_delayed_work(&vl6180_data->dwork, msecs_to_jiffies((vl6180_data->delay_ms)));	// restart timeR
		printk("%s reset finish force_reset_cnt(%d)\n",__func__,vl6180_data->force_reset_cnt);
		mutex_unlock(&vl6180_data->work_mutex);
		return;
	}
#endif
...
/* lanhe add for make sure calibration writting success*/
	if(offset_calib >= 3)
	{
		int8_t rd_offset = 0xff;
		VL6180x_RdByte(vl6180x_dev, SYSRANGE_PART_TO_PART_RANGE_OFFSET, &rd_offset);
		if(rd_offset != offset_calib/3)
		{
		   pr_err("%s rd_offset(%d) offset_calib(%d)\n", __func__,rd_offset,offset_calib);
		   VL6180x_SetOffset(vl6180x_dev,offset_calib);
		}
	}
	if(xtalk_calib > 0)
	{
	    uint16_t rd_xtalk=0xff;
		VL6180x_RdWord(vl6180x_dev, SYSRANGE_CROSSTALK_COMPENSATION_RATE, &rd_xtalk);
		if(rd_xtalk != xtalk_calib)
		{
		   pr_err("%s rd_xtalk(%d) rd_xtalk(%d)\n", __func__,rd_xtalk,xtalk_calib);
		   VL6180x_SetXTalkCompensationRate(vl6180x_dev, xtalk_calib);
		}
	}
#endif
...
/*zhengrong.zhang, 2015/07/08, add for laser*/
	...
	vl6180_data->delay_ms=50; //init to 20ms
...
int stmvl6180_power_enable(struct stmvl6180_data *vl6180_data, unsigned int enable)
{
	...
	if(enable) {
		...
		msleep(1);
		//lanhe add 2015/06/08
		stmvl6180_init_client(vl6180_data);
		msleep(5);
		rc = gpio_direction_output(vl6180_data->ce_gpio,0);
		if(rc){
			pr_err("%s:%d stmvl6180 gpip output failed rc=%d\n", __func__, __LINE__, rc);
